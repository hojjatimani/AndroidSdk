 ...
 * @author Kevin Gaudin
 */
final class ACRAConfigurationCollector {
    private static final String TAG = BefLog.TAG_PREF + "ACRAConfigurationCollector";
    ...
    private ACRAConfigurationCollector() {
        ...
        for (final Field f : Configuration.class.getFields()) {
            if (Modifier.isStatic(f.getModifiers()) && Modifier.isFinal(f.getModifiers())) {
                ...
                try {
                    ...
                    BefLog.w(TAG, "Error while inspecting device configuration: \n" + e.getMessage());
    ...
    private String toString(Configuration conf) {
        ...
        for (final Field f : conf.getClass().getFields()) {
            try {
                ...
                BefLog.e(TAG, "Error while inspecting device configuration: ", e);
            } catch (IllegalAccessException e) {
                BefLog.e(TAG, "Error while inspecting device configuration: ", e);
            }
        }
    ...
    public static String collectConfiguration(Context context) {
        try {
            final ACRAConfigurationCollector collector = new ACRAConfigurationCollector();
            final Configuration crashConf = context.getResources().getConfiguration();
            return collector.toString(crashConf);
        } catch (RuntimeException e) {
            BefLog.w(TAG, "Couldn't retrieve CrashConfiguration for : " + context.getPackageName() + "\n" + e.getMessage());
