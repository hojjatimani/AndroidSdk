 ...
 * file.
 */
final class ACRACrashReportPersister {
     ...
     * @return ACRACrashReportData read from the supplied InputStream.
     * @throws IOException if error occurs during reading from the {@code InputStream}.
     */
    public ACRACrashReportData load(File file) throws IOException {
     ...
     *
     * @param crashData     ACRACrashReportData to save.
     * @param file          File into which to store the ACRACrashReportData.
     * @throws IOException if the ACRACrashReportData could not be written to the OutputStream.
     */
    public void store( ACRACrashReportData crashData, File file) throws IOException {
        ...
        try {
            ...
            for (final Map.Entry<ACRAReportField, String> entry : crashData.entrySet()) {
     ...
     * </ul>
     *
     * @param reader    Reader from which to read the properties of this ACRACrashReportData.
     * @return ACRACrashReportData read from the supplied Reader.
     * @throws IOException if the properties could not be read.
     * @since 1.6
     */
    private synchronized ACRACrashReportData load(Reader reader) throws IOException {
        int mode = NONE, unicode = 0, count = 0;
        char nextChar, buf[] = new char[40];
        int offset = 0, keyLength = -1, intVal;
        boolean firstChar = true;

        final ACRACrashReportData crashData = new ACRACrashReportData();
            ...
            if (mode == SLASH) {
                ...
                case '\r':
                    ...
                    if (offset > 0 || (offset == 0 && keyLength == 0)) {
                        ...
                        crashData.put(Enum.valueOf(ACRAReportField.class, temp.substring(0, keyLength)), temp.substring(keyLength));
                    }
                    keyLength = -1;
                    offset = 0;
                    continue;
        ...
        if (keyLength >= 0) {
            final String temp = new String(buf, 0, offset);
            final ACRAReportField key = Enum.valueOf(ACRAReportField.class, temp.substring(0, keyLength));
        ...
        ACRACollectorUtil.safeClose(reader);
