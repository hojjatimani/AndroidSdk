...
class ACRAJSONReportBuilder {
    private static final String TAG = BefLog.TAG_PREF + "ACRAJSONReportBuilder";
    /**
     * <p>
     * Create a JSONObject containing the whole handleException data with the most
     ...
     *
     * @param errorContent
     *            The ACRA handleException data structure.
     * @return A JSONObject containing all fields from the handleException converted to
     *         JSON.
     * @throws JSONReportException
     */
    
    public static JSONObject buildJSONReport( ACRACrashReportData errorContent) throws JSONReportException {
        JSONObject jsonReport = new JSONObject();
        BufferedReader reader = null;
        for (ACRAReportField key : errorContent.keySet()) {
            try {
                // Each ACRAReportField can be identified as a substructure and not
                ...
                if (key.containsKeyValuePairs()) {
                    ...
                    try {
                        ...
                        BefLog.w(TAG, "Error while converting " + key.name() + " to JSON.", e);
                    }
        ...
        for (int i = 0; i < keys.length; i++) {
            ...
            if (i < keys.length - 1) {
                ...
                if (destination.isNull(subKey)) {
                    ...
                    if (intermediate == null) {
                        BefLog.w(TAG, "Unknown json subtree type, see issue #186");
                        // We should never get here, but if we do, drop this value to still send the handleException
                        return;
                    }
                }
